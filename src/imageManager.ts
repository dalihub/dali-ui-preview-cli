...
import { describeRegistry, alternateImage, BART_PROXY_HOST } from './registry';
...
export interface EnsureDeps {
    ...
    /** The OTHER registry's image for cross-registry fallback (BART⇄GHCR), or undefined. */
    alternateImage?: (image: string) => string | undefined;
    /** Create a local `docker tag` alias so a fallback-registry image is usable under the primary name. */
 ...
 */
export function analyzePullError(errorMessage: string): { category: PullErrorCategory } {
    const lower = errorMessage.toLowerCase();
    if (
        lower.includes('failed to authorize') || lower.includes('failed to fetch anonymous token') ||
        lower.includes('401') || lower.includes('403') || lower.includes('unauthorized') || lower.includes('denied')
    ) { return { category: 'auth' }; }
    if (
        lower.includes('x509') || lower.includes('certificate signed by unknown authority') ||
    ...
    if (
        lower.includes('no such host') || lower.includes('server misbehaving') ||
        lower.includes('name resolution') || lower.includes('could not resolve host')
    ) { return { category: 'dns' }; }
    if (
        lower.includes('connection refused') || lower.includes('connection reset') || lower.includes('timeout') ||
    ...
    if (
        lower.includes('not found') || lower.includes('manifest unknown') ||
 ...
 */
export function describeFailure(category: PullErrorCategory, host: string): { reason: string; fix: string } {
 ...
 */
export function buildDownloadFailureGuidance(attempts: { label: string; host: string; error: string }[]): string {
...
/** Result of the registry-fallback wrappers: the usable ref + which server served it. */
export interface RegistryFallbackResult {
    /** Image name now usable locally (always the RESOLVED/primary name — aliased if a fallback served it). */
    image: string;
    /** Tag that actually landed (may differ from the request when a tag fallback fired). */
    tag: string;
    /** The registry host the bytes actually came from (may be the fallback). */
    source: string;
}
 ...
 */
async function withRegistryFallback(
    image: string,
    tag: string,
    deps: EnsureDeps,
    inner: (image: string, tag: string, deps: EnsureDeps) => Promise<string>,
    ...
    const hosts: string[] = !alt
        ? [image]
        : describeRegistry(image).host === BART_PROXY_HOST
            ? [image, alt]
            : [alt, image]; // resolved to ghcr.io → try the BART mirror (alt) first

    const succeed = async (host: string, landed: string): Promise<RegistryFallbackResult> => {
        if (host !== image && deps.tagImage) {
            await deps.tagImage(`${host}:${landed}`, `${image}:${landed}`);
            deps.warn?.(`Tagged ${host}:${landed} -> ${image}:${landed}`);
        }
        return { image, tag: landed, source: describeRegistry(host).host };
    };

    for (let round = 1; round <= MAX_REGISTRY_ROUNDS; round++) {
        let anyRetryable = false;
        for (const host of hosts) {
            const desc = describeRegistry(host);
            try {
                ...
                if (analyzePullError(m).category === 'network') { anyRetryable = true; }
            }
        }
        if (!anyRetryable) { break; } // only hard/config errors this round — repeating can't help
...
export function ensureImageWithRegistryFallback(image: string, tag: string, deps: EnsureDeps): Promise<RegistryFallbackResult> {
...
export function pullWithRegistryFallback(image: string, tag: string, deps: EnsureDeps): Promise<RegistryFallbackResult> {
