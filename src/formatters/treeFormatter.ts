...
function formatBounds(node: MinimalNode): string {
 ...
 */
function formatLine(node: MinimalNode): string {
...
function appendNode(
    ...
    const childPrefix =
        connector === '' ? '' : prefix + (connector === TEE ? RAIL : BLANK);
 ...
 */
export function formatTree(root: MinimalNode): string {
    const out: string[] = [];
    appendNode(root, '', '', out);
    return out.join('\n');
}
