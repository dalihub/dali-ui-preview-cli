...
export async function dockerDaliVersion(image: string, tag: string): Promise<string> {
  try {
    const { stdout } = await execFileAsync(
      'docker',
      ['image', 'inspect', '--format', `{{ index .Config.Labels "${DALI_VERSION_LABEL}" }}`, `${image}:${tag}`],
      { timeout: 5000 },
    );
  ...
  return versionFromTag(tag) ?? UNKNOWN_VERSION;
}

/** dali-ui version of a native DALi prefix via `pkg-config --modversion`. */
export async function localDaliVersion(prefix: string): Promise<string> {
