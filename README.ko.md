...
> **어디에 맞는 도구인가 (그리고 정직한 한계):** 이 CLI는 *빠른 1차 검증 표면*입니다 — 에이전트가 직접 호출하고(stdout JSON, **MCP 서버 불필요**), 실기기 실행이 느리고 하드웨어가 귀한 곳(Tizen TV/임베디드)에서 가치가 가장 큽니다. 단, **기기 검증을 대체하지 않습니다**: 렌더는 추출된 영역의 headless 소프트웨어 래스터라 "프리뷰 통과"가 "기기에서 정확함"을 뜻하지 않습니다. 전체 근거: [`docs/value-and-positioning.md`](docs/value-and-positioning.md).
...
- **Linux (x86-64) 전용.** 이 CLI는 Linux Docker 런타임 이미지(또는 네이티브 DALi 빌드 + `g++`/`Xvfb`)로
  ...
  안내를 내고 멈춥니다. macOS·Windows에서는 Linux VM이나 원격 Linux 호스트를 쓰세요. (어떤 호스트든
...
| `14` | 지원하지 않는 호스트 OS(Linux 아님) — Linux 호스트에서 실행(macOS·Windows는 VM이나 원격 호스트). `--version`/`--help`는 어디서나 동작. |
