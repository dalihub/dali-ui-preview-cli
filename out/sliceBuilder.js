...
exports.findPreviewFunction = findPreviewFunction;
exports.scanRefs = scanRefs;
exports.collectSameFileDefs = collectSameFileDefs;
exports.synthWeakStub = synthWeakStub;
exports.buildSlice = buildSlice;
 ...
 */
function lineDirective(line0Based, srcPath) {
 ...
 */
function findPreviewFunction(src) {
...
function scanRefs(body) {
    ...
    // `auto x` / `auto& x`
    for (const lm of cleaned.matchAll(/\bauto\b[\s&*]+([A-Za-z_]\w*)/g)) {
        locals.add(lm[1]);
    }
    // primitive / std type declarations: `uint32_t colors[]`, `const char* names`, `int i`
    for (const lm of cleaned.matchAll(/\b(?:const\s+|unsigned\s+|signed\s+)*(?:u?int(?:8|16|32|64)?_t|int|float|double|bool|char|short|long|size_t|std::\w+)\b[\s*&]*([A-Za-z_]\w*)\s*[[=;,)]/g)) {
        locals.add(lm[1]);
    }
    // for-loop variable: `for (int i = …)` / `for (const auto& x : …)`
    for (const lm of cleaned.matchAll(/\bfor\s*\(\s*(?:const\s+)?[\w:]+\s*[&*]?\s*([A-Za-z_]\w*)\s*[=:]/g)) {
        locals.add(lm[1]);
    }
    ...
    for (const lm of cleaned.matchAll(/(?:^|[\n;{}])[ \t]*(?:const\s+)?(?:[A-Za-z_]\w*::)*[A-Z]\w*(?:<[^>{};]*>)?[\s&*]+([A-Za-z_]\w*)\s*[=;({]/g)) {
        locals.add(lm[1]);
    }
    ...
    while ((m = idRe.exec(cleaned)) !== null) {
        const id = m[2];
        if (KEYWORDS.has(id) || KNOWN_SYMBOLS.has(id) || locals.has(id)) {
            continue;
        }
        refs.add(id);
    }
 ...
 */
function collectSameFileDefs(src, refs, excludeRange, srcPath = '') {
    const collected = [];
    const unresolved = [];
...
function synthWeakStub(name, body) {
    const esc = name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
 ...
 */
function synthParamStub(type, name) {
    ...
    if (!bare.includes('*')) {
        bare = bare.replace(/^const\s+/, '');
    }
    if (/char\s*\*|\bstring\b|String/.test(bare)) {
        return `${bare} ${name} = "${humanizePlaceholder(name)}";`;
    }
    if (/\bbool\b/.test(bare)) {
        return `${bare} ${name} = false;`;
    }
    if (/\b(?:float|double)\b/.test(bare)) {
        return `${bare} ${name} = 0;`;
    }
    ...
    if (/\buint\w*\b|\bunsigned\b/.test(bare)) {
        return `${bare} ${name} = 0x888888;`;
    }
    if (/\b(?:int|short|long|size_t|signed)\b/.test(bare)) {
        return `${bare} ${name} = 0;`;
    }
    return `${bare} ${name}{};`;
}
 ...
 */
function parseMemberFields(sources) {
...
function parseStructFields(structText) {
    ...
    for (const decl of body.split(';')) {
        ...
        if (m) {
            fields.push({ type: m[1].trim(), name: m[2] });
        }
    }
    return fields;
}
 ...
 */
function synthSampleInit(type, structDefs, depth, fieldName) {
    if (depth > 4) {
        return '{}';
    }
    const bare = type.replace(/[&]/g, '').replace(/^const\s+/, '').trim();
    if (/char\s*\*|\bstring\b|String/.test(bare)) {
        return `"${fieldName ? humanizePlaceholder(fieldName) : 'Sample'}"`;
    }
    if (/\bbool\b/.test(bare)) {
        return 'true';
    }
    ...
    if (/\b(?:u?int\w*|short|long|size_t|unsigned|signed)\b/.test(bare)) {
        return '1';
    }
 ...
 */
function buildSlice(src, entrySrcPath, entryBody, extraSources = [], entryParams) {
    ...
    const allSources = [{ path: entrySrcPath, text: src }, ...extraSources];
    ...
    for (let round = 0; round < 4; round++) {
        ...
        for (const d of collected) {
            for (const nr of scanRefs(d.text)) {
                if (!have.has(nr) && !KNOWN_SYMBOLS.has(nr)) {
                    more.add(nr);
                }
            }
        }
    ...
    const memberStubs = memberRefs.map((m) => `__attribute__((weak)) ${baseTypeName(m.type)} ${m.name} = ${synthSampleInit(m.type, structDefs, 0, m.name)};`);
