...
function transformVectorChildren(code) {
    ...
    while ((m = CALL_RE.exec(code)) !== null) {
        ...
        let i = openParen + 1;
        while (i < code.length && isCppSpace(code[i])) {
            i++;
        }
        if (code[i] === '{') {
            ...
            let j = braceEnd + 1;
            while (j < code.length && isCppSpace(code[j])) {
                j++;
            }
            if (code[j] !== ')') {
                continue;
...
function transformLegacyFluentChildren(code) {
    return code
        // Legacy fluent return-expression form (pre-migration snippets):
